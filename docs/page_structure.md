...
- `checkin.html`: 当日受付。ラップトップを置き参加者が二次元コードをかざす前提(大カメラ+大判定・履歴なし)。
- `help.html`: 運営ヘルプ。
...
- `my.html`: **マイエントリー(参加者ハブ)**。1回の認証で 登録内容の確認 / 二次元コード再表示・保存 /
...
当日受付二次元コードは共有リンクではなく、確認メール・my.html・代理エントリー控えにのみ存在する。
